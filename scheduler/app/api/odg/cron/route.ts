...
interface CalendarConfig {
    odg: { calendarId: string; predefinito?: boolean }[];
}
interface SyncStats {
    scanned: number;
    inserted: number;
...
function getOdgCalendarId(): string {
    try {
        const configPath = path.join(process.cwd(), 'src', 'app', 'config', 'calendars.json');
        const fileContent = readFileSync(configPath, 'utf-8');
        const config: CalendarConfig = JSON.parse(fileContent);
        const odgCalendar = config.odg?.find(c => c.predefinito) || config.odg?.[0];
        if (!odgCalendar?.calendarId) {
            throw new Error("Nessun calendario ODG predefinito o valido trovato in /src/app/config/calendars.json");
        }
        return odgCalendar.calendarId;
    } catch (error: any) {
        throw new Error(`Impossibile leggere l'ID del calendario ODG: ${error.message}`);
    }
}

async function runSync(dryRun: boolean = false): Promise<{ ok: boolean; stats: SyncStats; details: DetailLog[]; error?: string; where?: string }> {
    const stats: SyncStats = { scanned: 0, inserted: 0, updated: 0, unchanged: 0, deleted: 0, skipped: 0 };
    const details: DetailLog[] = [];

    try {
        const calendarId = getOdgCalendarId();
        await log(`[CRON SYNC START] Calendar ID: ${calendarId}, Dry Run: ${dryRun}`);
            ...
            res.data.items?.forEach(event => {
                ...
                if (uid) {
                    if (existingEvents.has(uid)) {
                        ...
                        if (!dryRun) {
                            calendar.events.delete({ calendarId, eventId: event.id! }).catch(e => log(`ERROR: Failed to delete duplicate event ${event.id}: ${e.message}`));
                        }
                    } else {
        ...
        for (const [uid, eventData] of sourceEvents.entries()) {
            ...
            if (!existingEvent) {
                stats.inserted++;
                const logDetail = { odg_uid: uid, date: eventData.extendedProperties.private.odg_date_iso, action: 'insert' as const, reason: 'new event' };
                ...
                if (existingHash !== newHash) {
                    stats.updated++;
                    const logDetail = { odg_uid: uid, date: eventData.extendedProperties.private.odg_date_iso, action: 'update' as const, reason: 'content changed' };
                    details.push(logDetail);
                    ...
                    stats.unchanged++;
                    const logDetail = { odg_uid: uid, date: eventData.extendedProperties.private.odg_date_iso, action: 'skip' as const, reason: 'content unchanged' };
                    details.push(logDetail);
                    // Non logghiamo gli eventi invariati per non appesantire il log
                }
...
await log(`  - [DELETE] ${uid} (Date: ${dateToDelete}): Event not in source.`);
        ...
        await log(`[CRON SYNC END] Summary: Scanned=${stats.scanned}, Inserted=${stats.inserted}, Updated=${stats.updated}, Unchanged=${stats.unchanged}, Deleted=${stats.deleted}, Skipped=${stats.skipped}`);
        return { ok: true, stats, details };

    } catch (error: any) {
        const errorMessage = error.response?.data?.error?.message || error.message || 'An unknown error occurred.';
        await log(`[CRON SYNC FAILED] Error: ${errorMessage}`);
        return { ok: false, stats, details, error: errorMessage, where: "cron-sync" };
    }
}
...
export async function POST(request: Request) {
    try {
        const result = await runSync(false); // La cron non deve essere mai dry-run
        ...
        console.error("Fatal API Error in /api/odg/cron:", error);
        await log(`[FATAL ERROR] Cron job failed unexpectedly: ${error.message}`);
        return NextResponse.json(
            { ok: false, error: 'A fatal, unexpected error occurred in the cron handler.' },
            { status: 500 }
        );
    }
}
